...
const GUITAR_CHORDS = {
  ...
  'Db':   { frets: [-1,4,3,1,2,1], baseFret: 1, barre: 1 },
  'D':    { frets: [-1,-1,0,2,3,2], baseFret: 1, barre: null },
  'D#':   { frets: [-1,-1,1,3,4,3], baseFret: 1, barre: 1 },
  'Eb':   { frets: [-1,-1,1,3,4,3], baseFret: 1, barre: 1 },
  'E':    { frets: [0,2,2,1,0,0], baseFret: 1, barre: null },
  'F':    { frets: [1,1,2,3,3,1], baseFret: 1, barre: 1 },
  'F#':   { frets: [2,2,3,4,4,2], baseFret: 2, barre: 2 },
  'Gb':   { frets: [2,2,3,4,4,2], baseFret: 2, barre: 2 },
  'G':    { frets: [3,2,0,0,0,3], baseFret: 1, barre: null },
  'G#':   { frets: [4,3,1,1,1,4], baseFret: 1, barre: 1 },
  'Ab':   { frets: [4,3,1,1,1,4], baseFret: 1, barre: 1 },
  ...
  'D#m':  { frets: [-1,-1,1,3,4,2], baseFret: 1, barre: 1 },
  'Ebm':  { frets: [-1,-1,1,3,4,2], baseFret: 1, barre: 1 },
      ...
      {parts.map((p, i) =>
        p.type === 'chord' ? (
          <span
            ...
            className="text-primary font-bold hover:bg-primary/20 rounded transition-colors cursor-pointer hover:underline underline-offset-2"
...
export default function SongPreviewModal({ song, initialTab = 'chart', onClose, onEdit }) {
  ...
  const renderChart = () => {
    ...
    return displayChart.split('\n').map((line, i) => {
      ...
      return <div key={i} style={{ fontSize, fontFamily: 'monospace', whiteSpace: 'pre', lineHeight: '1.6' }} className="text-muted-foreground">{line}</div>;
    });
  };

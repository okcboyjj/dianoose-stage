...
Everything you need in one place. Copy each section into the corresponding file.
...
npx shadcn@latest add button input label textarea badge toast toaster
```
...
```
> Find your App ID in the Base44 editor URL: `base44.app/apps/YOUR_APP_ID/editor`

---

## STEP 2 — netlify.toml (root of project)
...
## STEP 3 — vite.config.js (root of project)
...
## STEP 4 — tailwind.config.js (root of project)
...
## STEP 7 — src/api/base44Client.js
...
## STEP 9 — src/lib/query-client.js
...
## STEP 11 — src/pages/ResetPassword.jsx
...
const GlobalStyles = () => (
  <style>{`
    ...
    .glass-panel { background: hsl(var(--card) / 0.8); backdrop-filter: blur(12px); border: 1px solid hsl(var(--border) / 0.5); }
  `}</style>
);
            ...
            <div className="space-y-4">
              {!token ? (
                <div className="text-center">
                  <button onClick={() => window.location.href = "/"} className="text-primary font-semibold hover:underline text-xs">← Back to Sign In</button>
                </div>
              ) : (
                <>
                  <div>
                    ...
                    <div className="relative mt-1.5">
                      <Input value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" type={showPassword ? "text" : "password"} className="bg-background/50 border-border/50 text-foreground text-sm pr-10" onKeyDown={e => e.key === "Enter" && handleReset()} />
                  ...
                  <div>
                    ...
                    <Input value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} placeholder="••••••••" type="password" className="mt-1.5 bg-background/50 border-border/50 text-foreground text-sm" onKeyDown={e => e.key === "Enter" && handleReset()} />
                  </div>
                  <Button onClick={handleReset} disabled={loading} className="w-full bg-primary text-primary-foreground hover:bg-primary/90 h-11 rounded-xl font-semibold shadow-lg shadow-primary/20 relative overflow-hidden mt-2">
...
## STEP 12 — src/components/app/NotificationsSection.jsx
...
export default function NotificationsSection({ notifications, onRefresh }) {
  ...
  const markAllRead = async () => {
    await Promise.all(notifications.filter(n => !n.is_read).map(n => NotificationEntity.update(n.id, { is_read: true })));
    onRefresh();
  };
...
## STEP 13 — src/components/app/MyStageSection.jsx
...
## STEP 14 — src/components/app/ServicesSection.jsx
  ...
  return (
    <div className="fixed inset-0 bg-background/80 backdrop-blur-md z-50 flex items-center justify-center p-4 overflow-y-auto">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-card border border-border/50 rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden my-auto">
        ...
        <div className="overflow-y-auto max-h-[60vh] p-6">
          {tab === "setlist" && (
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2 bg-background/50 border border-border/50 rounded-xl px-3 py-2 mb-3">
              ...
              <div className="space-y-1.5 max-h-40 overflow-y-auto">
                {availableSongs.slice(0, 20).map(song => (
                  <button key={song.id} onClick={() => addSong(song.id)} className="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl bg-background/30 hover:bg-primary/10 border border-transparent transition-all text-left group">
          ...
          {tab === "info" && (
            <div className="space-y-4">
              ...
              <div>
                <Label className="text-xs font-medium text-muted-foreground ml-1">Type</Label>
                <select value={form.type || "Sunday Morning"} onChange={e => set("type", e.target.value)} className="mt-1.5 w-full bg-background/50 border border-border/50 text-foreground text-sm rounded-md px-3 py-2.5 outline-none">
                  {SERVICE_TYPES.map(t => <option key={t} value={t} className="bg-card">{t}</option>)}
                </select>
              </div>
              <div>
                <Label className="text-xs font-medium text-muted-foreground ml-1">Status</Label>
                <select value={form.status || "upcoming"} onChange={e => set("status", e.target.value)} className="mt-1.5 w-full bg-background/50 border border-border/50 text-foreground text-sm rounded-md px-3 py-2.5 outline-none">
        ...
        <div className="flex gap-3 px-6 py-4 border-t border-border/50 bg-secondary/10">
          <button onClick={handleDelete} className="flex items-center gap-1.5 text-xs text-destructive hover:underline font-medium"><Trash2 className="w-3.5 h-3.5" /> Delete</button>
          ...
          {filtered.map(service => {
            ...
            return (
              <div key={service.id} onClick={() => setSelectedService(service)} className="glass-panel rounded-xl px-5 py-4 hover:border-primary/50 transition-all duration-300 cursor-pointer group flex items-center gap-5 hover:shadow-lg hover:-translate-y-0.5">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${isUpcoming ? "bg-primary/10 group-hover:bg-primary/20" : "bg-secondary/50"}`}>
...
## STEP 15 — src/components/app/MyLibrarySection.jsx
    ...
    <div className="fixed inset-0 bg-background/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-card border border-border/50 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
        ...
        <div className="p-6 space-y-4">
          ...
          <div><Label className="text-xs font-medium text-muted-foreground ml-1">Personal Notes</Label><Textarea value={form.personal_notes} onChange={e => set("personal_notes", e.target.value)} placeholder="My arrangement notes, cues..." rows={4} className="mt-1.5 bg-background/50 border-border/50 text-foreground text-sm resize-none" /></div>
          <div>
            <Label className="text-xs font-medium text-muted-foreground ml-1">Category</Label>
            <select value={form.category} onChange={e => set("category", e.target.value)} className="mt-1.5 w-full bg-background/50 border border-border/50 text-foreground text-sm rounded-md px-3 py-2.5 outline-none">
          ...
          {filtered.map(entry => (
            <div key={entry.id} onClick={() => setSelectedEntry(entry)} className="glass-panel rounded-xl px-5 py-4 hover:border-primary/50 transition-all duration-300 cursor-pointer group flex items-center gap-5 hover:shadow-lg hover:-translate-y-0.5">
              ...
              <div className="flex items-center gap-3 shrink-0">
                ...
                {entry.rating > 0 && (
                  <div className="flex gap-0.5">{[1,2,3,4,5].map(n => <Star key={n} className={`w-3 h-3 ${entry.rating >= n ? "text-primary fill-primary" : "text-muted-foreground/30"}`} />)}</div>
                )}
              </div>
            </div>
          ))}
...
## STEP 16 — src/components/app/MusicianSection.jsx
...
function MemberModal({ member, churchId, onClose, onSave }) {
  ...
  const handleSave = async () => {
    ...
    if (isNew && !form.password.trim()) { setError("A temporary password is required."); return; }
    setSaving(true);
    try {
      if (isNew) {
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-card border border-border/50 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
        ...
        <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          <div>
            ...
            <select value={form.role} onChange={e => set("role", e.target.value)} className="mt-1.5 w-full bg-background/50 border border-border/50 text-foreground text-sm rounded-md px-3 py-2.5 outline-none">
              {ROLES.map(r => <option key={r} value={r} className="bg-card">{r}</option>)}
            </select>
          </div>
          {isNew && <div><Label className="text-xs font-medium text-muted-foreground ml-1">Temporary Password *</Label><Input value={form.password} onChange={e => set("password", e.target.value)} type="password" placeholder="They'll use this to sign in" className="mt-1.5 bg-background/50 border-border/50 text-foreground text-sm" /></div>}
          <div>
...
## STEP 17 — src/components/app/AdminSection.jsx
...
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Shield, Users, Copy, Check, RefreshCw, Loader2, Share2, X, Plus, Trash2, Save } from "lucide-react";
    ...
    <div className="fixed inset-0 bg-background/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-card border border-border/50 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
        ...
        <div className="p-6 space-y-5">
          ...
          <div>
            <Label className="text-xs font-medium text-muted-foreground ml-1 block mb-2">Your Team Code</Label>
...
function AddMemberModal({ churchId, onClose, onSave }) {
  const [form, setForm] = useState({ first_name: "", last_name: "", email: "", instrument: "", role: "Musician", password: "" });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  const set = (k, v) => setForm(f => ({ ...f, [k]: v }));
  ...
  const handleSave = async () => {
    ...
    try {
      await base44.auth.register({ email: form.email.trim(), password: form.password });
      await ChurchMemberEntity.create({ first_name: form.first_name.trim(), last_name: form.last_name.trim(), email: form.email.trim(), instrument: form.instrument.trim(), role: form.role, church_id: churchId, is_active: true, user_id: "" });
      onSave();
    } catch (e) {
      const msg = (e?.message || "").toLowerCase();
      if (msg.includes("already") || msg.includes("exists") || msg.includes("registered")) {
    ...
    <div className="fixed inset-0 bg-background/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-card border border-border/50 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
        ...
        <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          <div><Label className="text-xs font-medium text-muted-foreground ml-1">Temporary Password *</Label><Input value={form.password} onChange={e => set("password", e.target.value)} type="password" className="mt-1.5 bg-background/50 border-border/50 text-sm" /></div>
        </div>
        <div className="flex gap-3 px-6 py-4 border-t border-border/50 bg-secondary/10">
          <Button variant="outline" onClick={onClose} className="border-border/50 text-foreground hover:bg-background h-10 px-5 rounded-lg">Cancel</Button>
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="glass-panel rounded-2xl p-6">
        ...
        <Button onClick={() => setShowCodeModal(true)} variant="outline" className="border-primary/30 text-primary hover:bg-primary/10 h-10 rounded-xl font-semibold">
...
## STEP 18 — src/components/app/SettingsSection.jsx
        ...
        <div className="glass-panel rounded-2xl p-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <select value={schedForm.service_day} onChange={e => setSc("service_day", e.target.value)} className="mt-1.5 w-full bg-background/50 border border-border/50 text-foreground text-sm rounded-md px-3 py-2.5 outline-none">
            ...
            <div>
              ...
              <select value={schedForm.timezone} onChange={e => setSc("timezone", e.target.value)} className="mt-1.5 w-full bg-background/50 border border-border/50 text-foreground text-sm rounded-md px-3 py-2.5 outline-none">
...
## STEP 19 — src/pages/Home.jsx

**Copy the entire content of `pages/Home` from the Base44 editor** — it's the large file already in your context snapshot. It contains: LoginScreen, VerifyEmailScreen, SetupWizard, CountdownTimer, SongModal, MainApp, and the root Home export.
...
#    VITE_BASE44_APP_ID = (your app ID from Base44 editor URL)
# 6. Deploy!
```

**That's it — the entire app, ready to run on Netlify with Base44 as the backend.**